...
a<-read.csv("11529_e_eva.csv", stringsAsFactors = F)
a<-a[!is.na(a$mutAA),]
a.f<-f.out(a)

b<-read.csv("11247_e_eva.csv", stringsAsFactors = F)
b<-b[!is.na(b$mutAA),]
b.f<-f.out(b)

c<-read.csv("11463_e_eva.csv", stringsAsFactors = F)
c<-c[!is.na(c$mutAA),]
c.f<-f.out(c)

g<-read.csv("11485_e_eva.csv", stringsAsFactors = F)
g<-g[!is.na(g$mutAA),]
g.f<-f.out(g)

h<-read.csv("11362_e_eva.csv", stringsAsFactors = F)
h<-h[!is.na(h$mutAA),]
h.f<-f.out(h)

#k<-read.csv("11337_e_eva.csv", stringsAsFactors = F)
#k<-k[!is.na(k$mutAA),]
#k.f<-f.out(k)
...
i<-rbind(a.f,c.f,b.f,h.f,g.f,j.f)#,k.f)#,j.f) #,e.f,h.f,m.f,o.f) b.f, ,h.f,g.f
...
# Save output
write.csv(d[,c("resnum", "wt", "res", "Mut", "score", "score_SE", "type", "var")],"C:/Users/KRONCKE/OneDrive - VUMC/Kroncke_Lab/kcnh2/sequencing/VANTAGE/DrugRescue/alex/Tile3-eva.csv", row.names = F)
