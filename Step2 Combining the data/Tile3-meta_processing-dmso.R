...
a<-read.csv("11529_e_dmso.csv", stringsAsFactors = F)
a<-a[!is.na(a$mutAA),]
a.f<-f.out(a)

b<-read.csv("11247_e_dmso.csv", stringsAsFactors = F)
b<-b[!is.na(b$mutAA),]
b.f<-f.out(b)

c<-read.csv("11463_e_dmso.csv", stringsAsFactors = F)
c<-c[!is.na(c$mutAA),]
c.f<-f.out(c)

g<-read.csv("11485_e_dmso.csv", stringsAsFactors = F)
g<-g[!is.na(g$mutAA),]
g.f<-f.out(g)

j<-read.csv("C:/Users/KRONCKE/OneDrive - VUMC/Kroncke_Lab/kcnh2/sequencing/VANTAGE/pilot/Drug_Variant_Interaction/egly/9934/9934_e_dmso.csv", stringsAsFactors = F)
j<-j[!is.na(j$mutAA),]
names(j)[names(j) == "firstCount"] <- "count"
j.f<-f.out(j)


#h<-read.csv("Library3-LV384/DMS_LV384_8649_1_e.csv", stringsAsFactors = F)
#h<-h[!is.na(h$mutAA),]
#h.f<-f.out(h)

#k<-read.csv("Library3-LV384/DMS_LV384_8649_2_e.csv", stringsAsFactors = F)
#k<-k[!is.na(k$mutAA),]
#k.f<-f.out(k)

#j<-read.csv("Library3-LV384/DMS_LV384_8649_3_e.csv", stringsAsFactors = F)
#j<-j[!is.na(j$mutAA),]
#j.f<-f.out(j)

i<-rbind(a.f,b.f,c.f,g.f,j.f)#,h.f,k.f,j.f) #,e.f,h.f,m.f,o.f)
...
# Save output
write.csv(d[,c("resnum", "wt", "res", "Mut", "score", "score_SE", "type", "var")],"C:/Users/KRONCKE/OneDrive - VUMC/Kroncke_Lab/kcnh2/sequencing/VANTAGE/DrugRescue/alex/Tile3-dmso.csv", row.names = F)
